...
Rasmus Tuokko, Hervanta
rasmus.tuokko@tuni.fi

Opiskelijanumero: 150390123
Lassi Lappalainen, Hervanta
...
def add_friendship(network, name1, name2):
    ...
    for name in [name1, name2]:
        ...
        if name1 == name2:
            print(f"Error: '{name}' can't be added as his or her own friend")
            return
...
def common_friends_command(network, namelist):
    ...
    network[name1] = sorted(network[name1])
    network[name2] = sorted(network[name2])

    for index in range(len(network[name2])):
        if network[name2][index] in network[name1]:
            common_friends_list.append(network[name2][index])

    if not common_friends_list:
        print(f"{name1} and {name2} have no common friends.")
    else:
        for index in range(0, len(common_friends_list)):
            print(common_friends_list[index])
