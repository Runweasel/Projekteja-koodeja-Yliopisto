...
public:
     ...
     * @brief sign_up
     * @param new_student
     * Add a student to course. Prints error message if the student have already
     * completed the course or is already part of the course.
     */
    void sign_up(Account* new_student_member);

    /**
     * @brief get_course_students
     * @return the signed students of the course.
     */
    std::vector<Account*> get_course_students() const;

    /**
     * @brief get_course_name
...
private:
    ...
    std::string name_;
    int credits_; // Kurssin opintopisteet
    std::vector<Account*> course_staff_; // Kurssin henkilökunta
    std::vector<Account*> course_students_; // Kurssin opiskelijat
};
