...
Tämä ohjelma määrittelee muuttolaatikoita ja niiden sisältöä.
Muuttolaatikoille voi tehdä lukuisia eri operaatioita, kuten
lisätä niihin tavaroita, poistaa tavaroita, siirtää tavaroita
toiseen laatikkoon jne.

Muuttolaatikolle tehdään operaatioita luokalla MovingBox ja
sen metodien avulla.
"""
...
class MovingBox:
    ...
    def add_item(self, item_name, item_count):
        if item_name in self.dic_box:
            item_count = self.dic_box.get(item_name) + item_count
            self.dic_box.update({item_name: item_count})
            return
    ...
    def transfer_item(self, target_name, item_name, item_count):
        if item_name in self.dic_box:
            box1_item_count = self.dic_box[item_name] - item_count

            if box1_item_count < 0:
                return False

            self.dic_box.update({item_name: box1_item_count})

            if self.dic_box[item_name] == 0:
                del self.dic_box[item_name]

            if item_name not in target_name.dic_box:
                target_name.dic_box.update({item_name: item_count})
                return True
            else:
                box2_item_count = target_name.dic_box[item_name] + item_count
                target_name.dic_box.update({item_name: box2_item_count})
                return True
    ...
    def list_content(self):
        item_count = 0
        for item_name in self.dic_box:
            item_count = self.dic_box[item_name] + item_count

        print(f"Box \"{self.box_name}\" contains {item_count} items.")
        for item_name in sorted(self.dic_box):
            print(f"{self.dic_box[item_name]:3} {item_name}")

        return

    def search_item(self, item_name):
        if item_name in self.dic_box:
            return self.dic_box[item_name]
        else:
            return False
